...
use raving::vk::{GpuResources, VkEngine};

use crate::console::Console;
use crate::util::BufferStorage;
...
impl TreeList {
    ...
    pub fn update_layer(
        ...
        let targeted_crumb =
            compositor.with_layer::<_, _>(&layer_name, |layer| {
                ...
                if let Some(sublayer) =
                    layer.get_sublayer_mut(&self.sublayer_text)
                {
                ...
                if let Some(sublayer) =
                    layer.get_sublayer_mut(&self.sublayer_rect)
                {
                    let w = 4.0 + max_width;
                    ...
                    bg[16..32].clone_from_slice(
                        [0.85f32, 0.85, 0.85, 1.0].as_bytes(),
                    );

                    for draw_data in sublayer.draw_data_mut() {
                        draw_data.update_vertices_array_range(0..1, [bg])?;
                        ...
                        draw_data.update_vertices_array(
                            Some(bg)
                                ...
                                .chain(
                                    targeted_crumb.iter().cloned().flat_map(
                                        |(crumb, [x, y, w, h])| {
                                            let mut up = [0u8; 32];

                                            let color = [1f32, 0.0, 0.0, 1.0];
                                            up[16..32].clone_from_slice(
                                                color.as_bytes(),
                                            );
                                            ...
                                            left[8..16].clone_from_slice(
                                                [1.0, h].as_bytes(),
                                            );

                                            let mut down = up;
                                            down[4..8].clone_from_slice(
                                                [y + h].as_bytes(),
                                            );

                                            let mut right = left;
                                            right[0..4].clone_from_slice(
                                                [x0 + w].as_bytes(),
                                            );
        ...
        if let Some((target, _)) = targeted_crumb {
            if mouse_clicked {
                if let Some(val) = target.index_dyn_ref(source) {
                    return Ok(Some((target, val)));
                } else {
                    return Ok(Some((target, rhai::Dynamic::UNIT)));
                }
            }
        }
